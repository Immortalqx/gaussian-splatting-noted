...
# 注释参考：https://github.com/shuttworth/3DGS_Noted
# 主函数, 训练函数由五个主步骤
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # 在指定迭代区间内，对3D高斯模型进行致密化and剪枝
            # Densification
            if iteration < opt.densify_until_iter:
                # 更新高斯模型中可见点的最大半径，这是为了后续的修剪操作。这里使用了一个可见性过滤器来选择特定的点
...
if __name__ == "__main__":
    ...
    # 使用 parser.add_argument 方法添加了多个命令行参数，
    # 包括IP地址、端口号、调试起点、是否检测异常、测试和保存迭代次数、是否安静模式（不输出信息）、检查点迭代次数和开始检查点的路径
